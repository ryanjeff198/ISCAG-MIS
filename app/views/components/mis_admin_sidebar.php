...
<?php
  // Fetch live unread count for sidebar badge
  $sidebarUnread = 0;
  try {
      require_once BASE_PATH . '/app/models/AdminNotification.php';
      $sidebarUnread = (new AdminNotification())->getUnreadCount();
  } catch (\Exception $e) { /* silently fail */ }
?>
        <a href="<?= url('/admin/mis_admin/notification') ?>" class="nav-item <?= $active_page === 'notification' ? 'active' : '' ?>" style="position:relative;">
            ...
            <span class="notif-dot" style="position:absolute;top:6px;right:10px;background:#ef4444;color:#fff;font-size:0.6rem;font-weight:700;padding:1px 6px;border-radius:10px;min-width:16px;text-align:center;"><?= $sidebarUnread > 99 ? '99+' : $sidebarUnread ?></span>
            <?php endif; ?>
        </a>
    </nav>
